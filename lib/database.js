...
Database.prototype.initialize = function (cb) {
    ...
    if (!self.updateWatcher) {
        self.updateWatcher = fsx.watchFile(self.filePath, {
            interval: 100
        }, function () {
            if (self.ownWrite) {
                self.ownWrite = false;
...
Database.prototype.write = function (collectionName, cb) {
    ...
    fsx.outputJsonSync(self.filePath, {
        data: data,
        schema: schema,
        counters: counters
    });
...
Database.prototype.read = function (cb) {
    ...
    var data = fsx.readFileSync(this.filePath, {
        encoding: 'utf8'
    });
